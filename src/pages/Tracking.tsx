...
interface TrackedVehicle {
  id: string; // dispatch id OR `veh-${vehicleId}` for offline
...
const TrackingPage = () => {
  ...
  // All vehicles in the org (for offline fallback)
  ...
  // Drivers lookup (only for the dispatched ones)
  ...
  // Realtime subscription for live dispatch updates
  useEffect(() => {
    if (!organizationId) return;
    const channel = supabase
      .channel(`tracking-${organizationId}`)
      .on(
        "postgres_changes",
  ...
  const vehicles: TrackedVehicle[] = useMemo(() => {
        ...
        speed: uiStatus === "active" && d.distance_km && d.scheduled_delivery && d.scheduled_pickup
          ? Math.round(
              (Number(d.distance_km) /
                Math.max(0.5, (new Date(d.scheduled_delivery).getTime() - new Date(d.scheduled_pickup).getTime()) / 3600000)),
            )
          : 0,
    ...
    // Offline fallback: every vehicle in this tenant without an active dispatch
  ...
  const selectedVehicle =
    vehicles.find((v) => v.id === selectedId) || pagedVehicles[0] || filteredVehicles[0] || null;

  const activeCount = vehicles.filter((v) => v.status === "active").length;
  const idleCount = vehicles.filter((v) => v.status === "idle").length;
  const offlineCount = vehicles.filter((v) => v.status === "offline").length;

  // Status-update pins (geocoded by edge function)
            ...
            >
              ...
              {selectedVehicle.hasDispatch ? (
                ...
                <div className="pt-3 border-t border-border/50 text-sm text-muted-foreground">
                  This vehicle has no active dispatch and is currently <span className="text-foreground font-medium">offline</span>.
                  It will appear with live shipment details once a dispatch is assigned.
                </div>
              )}
            </motion.div>
          )}
