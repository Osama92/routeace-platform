...
export default function AdvancedRoutePlanner() {
  ...
  const confirmApplyToDispatch = async () => {
    ...
    try {
      ...
      // Insert intermediate stops as dispatch_dropoffs
  ...
  // ── Export PDF (print) ──────────────────────────────────────────────────────
  // Uses window.print() with an injected print stylesheet — no external library.
  const handleExportPdf = () => {
    ...
    const stopRows = stops.map((s, i) =>
      `<tr><td>${i + 1}</td><td>${s.label || `Stop ${i + 1}`}</td><td>${s.address}</td><td>${s.weightKg?.toLocaleString() ?? "—"} kg</td></tr>`
    ).join("");

    const html = `
      <!DOCTYPE html><html><head><title>Route Plan — ${opt.name}</title>
      <style>
        ...
        .kpi-box { border: 1px solid #ddd; border-radius: 6px; padding: 8px; }
        .kpi-box .label { font-size: 10px; color: #666; }
        .kpi-box .val { font-size: 14px; font-weight: bold; }
        @media print { @page { size: A4; margin: 16mm; } }
      </style></head><body>
      <h1>Route Plan — ${opt.name}</h1>
      <div class="sub">Generated by RouteAce · ${new Date().toLocaleString("en-NG")} · Algorithm: ${opt.algorithm}</div>
      <div class="kpi">
        <div class="kpi-box"><div class="label">From</div><div class="val">${origin.address}</div></div>
        <div class="kpi-box"><div class="label">To</div><div class="val">${destination.address}</div></div>
        <div class="kpi-box"><div class="label">Vehicle</div><div class="val">${vehicle ? `${vehicle.registration_number} (${vehicle.vehicle_type})` : mode.label}</div></div>
        <div class="kpi-box"><div class="label">Distance</div><div class="val">${opt.distanceKm} km</div></div>
        <div class="kpi-box"><div class="label">Est. Duration</div><div class="val">${opt.durationHours} hrs</div></div>
        <div class="kpi-box"><div class="label">Total Cost</div><div class="val">₦${opt.totalCost.toLocaleString()}</div></div>
        <div class="kpi-box"><div class="label">Fuel Cost</div><div class="val">₦${opt.fuelCost.toLocaleString()}</div></div>
        <div class="kpi-box"><div class="label">Est. Revenue</div><div class="val">₦${opt.estimatedRevenue.toLocaleString()}</div></div>
        <div class="kpi-box"><div class="label">Profit Margin</div><div class="val">${opt.profitMargin}%</div></div>
      </div>
      ${stops.length > 0 ? `
        <table><thead><tr><th>#</th><th>Label</th><th>Address</th><th>Weight</th></tr></thead>
        <tbody>${stopRows}</tbody></table>` : ""}
      ${opt.overloadWarning ? `<p style="color:#b45309;font-weight:bold">${opt.overloadWarning}</p>` : ""}
      ${opt.marginWarning   ? `<p style="color:#b45309;font-weight:bold">${opt.marginWarning}</p>`   : ""}
      <p style="margin-top:24px;color:#888;font-size:10px">Confidence Score: ${opt.confidenceScore}% · SLA Risk: ${opt.slaRisk} · Cargo: ${cargoType}</p>
      </body></html>`;

    const win = window.open("", "_blank", "width=800,height=600");
    if (!win) {
      toast({ title: "Pop-up blocked", description: "Allow pop-ups for this site to export the route PDF.", variant: "destructive" });
      return;
    }
  ...
  // ── Send to Driver ──────────────────────────────────────────────────────────
  // Inserts a driver_notification row. If no driver is assigned yet, prompts
  // the user to select the fleet vehicle first.
  ...
  const handleSendToDriver = async () => {
    ...
    if (!vehicle) {
      toast({
        ...
        description: "Select a fleet vehicle in the planner first — the route will be sent to that vehicle's assigned driver.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const routeSummary = [
        ...
        `Distance: ${opt.distanceKm} km · Est. ${opt.durationHours} hrs`,
        `Route: ${opt.name} (${opt.algorithm})`,
      ].join("\n");

      await (supabase.from("driver_notifications") as any).insert({
        organization_id: organizationId,
        vehicle_id:      vehicle.id,
        notification_type: "route_assignment",
        title:           `Route assigned — ${origin.address.split(",")[0]} → ${destination.address.split(",")[0]}`,
        message:         routeSummary,
        metadata: {
          route_name:    opt.name,
          distance_km:   opt.distanceKm,
          duration_hrs:  opt.durationHours,
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:gap-3">
                <Button
                  ...
                  disabled={hasComplianceBlocking || applyingToDispatch}
                ...
                <div className="flex gap-2 sm:gap-3">
                  ...
                  <Button
                    variant="outline"
                    className="flex-1 sm:flex-none"
                    disabled={sendingToDriver}
                    onClick={handleSendToDriver}
                  >
                    {sendingToDriver
                      ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Sending…</>
                      : <><ArrowRight className="w-4 h-4 mr-2" />Send to Driver</>
                    }
                  </Button>
                </div>
              </div>
