...
const PendingApprovalScreen = () => {
  const { signOut, user, refreshApprovalStatus } = useAuth();
  const [checking, setChecking] = useState(false);

  // Auto-poll every 5 seconds - if admin approves, user gets in automatically
  useEffect(() => {
    if (!user) return;
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      ...
      >
        ...
        <p className="text-muted-foreground mb-6">
          Your account is awaiting approval. Once your administrator approves you,
        ...
        <div className="border-t border-border pt-6 space-y-3">
          ...
          <p className="text-xs text-muted-foreground pt-2">
            Checking every few seconds. Need help? Contact your administrator.
          </p>
        </div>
      </motion.div>
    </div>
