        ...
        .setPopup(
          new mapboxgl.Popup({ offset: 25 }).setHTML(`
            <div class="p-2 bg-background text-foreground">
              ...
              <p class="text-xs mt-1">${escHtml(pickup.address)}</p>
            </div>
          `)
        )
        .addTo(map.current!);
          ...
          .setPopup(
            new mapboxgl.Popup({ offset: 25 }).setHTML(`
              <div class="p-2 bg-background text-foreground">
                ...
                <p class="text-xs mt-1">${escHtml(dropoff.address)}</p>
                ${dropoff.label ? `<p class="text-xs text-gray-500">${escHtml(dropoff.label)}</p>` : ""}
              </div>
            `)
          )
          .addTo(map.current!);
        ...
        .setPopup(
          new mapboxgl.Popup({ offset: 25 }).setHTML(`
            <div class="p-2 bg-background text-foreground">
              ...
              <p class="text-xs mt-1">${escHtml(delivery.address)}</p>
            </div>
          `)
        )
        .addTo(map.current!);
