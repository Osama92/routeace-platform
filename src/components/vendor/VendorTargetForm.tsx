...
const VendorTargetForm = ({ onSaveComplete }: VendorTargetFormProps) => {
  ...
  const fetchVendors = async () => {
    // Fetch all vendors regardless of approval status (some may not have approval_status set)
              ...
              <SelectContent>
                {vendors.length === 0 ? (
                  <div className="px-2 py-4 text-center text-sm text-muted-foreground">
                    No vendors found. Add vendors in the Partners section first (select "Vendor" as partner type).
                  </div>
                ) : (
                  vendors.map((vendor) => (
