...
const CustomerProfitabilityReport = () => {
  ...
  const fetchCustomerDispatches = async (customerId: string) => {
    ...
    try {
      // Fetch dispatches for this customer - org-scoped
      const { data: dispatches } = await supabase
        .from("dispatches")
      ...
      // Fetch COGS expenses linked to these dispatches with category breakdown - org-scoped
      ...
      // Fetch invoices for this customer - org-scoped
      const { data: invoices } = await supabase
        .from("invoices")
