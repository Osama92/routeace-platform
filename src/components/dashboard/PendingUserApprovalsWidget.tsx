...
const PendingUserApprovalsWidget = () => {
  ...
  const { hasRole, organizationId, isSuperAdmin } = useAuth();

  const isAdmin = hasRole("admin") || hasRole("super_admin") || hasRole("org_admin");
  ...
  useEffect(() => {
    ...
    const fetchPendingUsers = async () => {
      ...
      // Always scope to the admin's own workspace - including Super Admins.
      // Each Super Admin owns their own tenant and must not see other tenants' pending users.
      if (!organizationId) {
        setPendingUsers([]);
        setLoading(false);
        return;
