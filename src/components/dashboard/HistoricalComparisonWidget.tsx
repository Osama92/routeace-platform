  ...
  const fetchComparisonData = async () => {
    ...
    try {
      ...
      const { data: historicalData, error: historicalErr } = await supabase
        ...
        .select("trips_count, total_revenue, total_cost")
      ...
      if (historicalData && historicalData.length > 0) {
        ...
        // Estimate distance based on average trip distance
        previousDistance = previousTrips * 300; // Rough average
      } else {
        // Fall back to dispatches from last year
