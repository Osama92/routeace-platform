-- ============================================================
-- A super admin can approve a rate change but couldn't propose one
-- ============================================================
-- Reported from the field: a super admin editing distance_km on an already-
-- approved rate got "Only finance or an org admin can propose a rate
-- change" — approve_rate_card() correctly requires is_super_admin() OR
-- is_platform_owner(), but propose_rate_card_change() only allowed
-- finance_manager/org_admin/admin. So the one role that outranks all of
-- those everywhere else in the platform was the one role that could approve
-- a change but not start one for their own org.
--
-- Fix: add is_super_admin() / is_platform_owner() to the same check
-- approve_rate_card() and reject_rate_card() already use, so all three
-- functions agree on who the top tier is.
-- ============================================================
